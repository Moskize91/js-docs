...
`room`的`state`中，存在`memberState`属性（可以阅读[状态管理](/documents/client/realtime-room-state-management)查看更多`state`信息）。 普通教具都可以通过`memberState`进行描述。
...
1. 在初始化时，根据[初始化参数-房间参数](/javascript/initialization/room-parameters)中的`disableEraseImage`字段配置:`是否可以擦除图片（默认可以）`
...
请阅读[初始化参数-SDK参数](/javascript/initialization/sdk-parameters)中的`handToolKey`字段说明。
...
当抓手工具被激活（包括主动激活）时，会回调[初始化参数-房间参数](/javascript/initialization/room-parameters)中`callbacks`的`onHandToolActive` 方法。
